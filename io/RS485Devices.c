...
unsigned int readRS485AnalogRecorder(unsigned short RS485Chan, unsigned short AnalogChan, float vref, float* volts, float* stdev){
	...
	float tempf;
		cmd[0]=RS485Chan;
		cmd[1]=0x08;  // This command code is proprietery to our equipment. Expected return is 4 bytes. for these
...
//		printf("Raw sum of deviations: (10*Xi - 10*Xavg)² = %d\n",temp);
		tempf =((float) temp) / 100.0;
		tempf = sqrt(tempf/9.0);  // should this be stderror instead of stdev?
...
int sendGPIBData(unsigned char *cmd, char gpibaddress, unsigned short Address){
	...
	outData[length+5]=13; //append a CR
	// calculate the CRC
