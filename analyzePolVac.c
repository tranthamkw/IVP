...
//	printf("%f\t%f\t%f\t%f\t%f\t%f\n",cv_a[k],c0_a[k],c2_a[k],s2_a[k],c4_a[k],s4_a[k]);
	...
	if (printHeader){
		if(printSummary==1){
		fprintf(fp_out,"SourceFile\tComment\t<CVG>\t±CVG\t<LED>\t±LED\t<T>\t±T\t<M/I>\t±M/I\t<C/I>\t±C/I\t<S/I>\t±S/I\n");
		printf("<CVG>\t±CVG\t<LED>\t±LED\t<T>\t±T\t<M/I>\t±M/I\t<C/I>\t±C/I\t<S/I>\t±S/I\n");
		}
		if(printSummary==0){
		 fprintf(fp_out,"Sourcefile\tComments\tCVG\tLED\tTemp\tMI\tCI\tSI\n");
		 printf("Sourcefile\tComments\tCVG\tLED\tTemp\tMI\tCI\tSI\n");
		}
	}
...
if (printSummary==1){
	fprintf(fp_out,"%s\t",inFileName);
	fprintf(fp_out,"%s\t",comments);
	calculateStats(&avg,&stdvm,0,25,cv_p);
	fprintf(fp_out,"%f\t%f\t",avg,stdvm);
		printf("%2.2E %2.2E ",avg,stdvm);
	calculateStats(&avg,&stdvm,0,25,led_p);
	fprintf(fp_out,"%f\t%f\t",avg,stdvm);
		printf("%2.2E %2.2E ",avg,stdvm);
	calculateStats(&avg,&stdvm,0,25,temp_p);
	fprintf(fp_out,"%f\t%f\t",avg,stdvm);
		printf("%2.2E %2.2E ",avg,stdvm);
	calculateStats(&avg,&stdvm,0,25,MoverI);
	fprintf(fp_out,"%f\t%f\t",avg,stdvm);
		printf("%2.2E %2.2E ",avg,stdvm);
	calculateStats(&avg,&stdvm,0,25,CoverI);
	fprintf(fp_out,"%f\t%f\t",avg,stdvm);
		printf("%2.2E %2.2E ",avg,stdvm);
	calculateStats(&avg,&stdvm,0,25,SoverI);
	fprintf(fp_out,"%f\t%f\n",avg,stdvm);
		printf("%2.2E %2.2E\n",avg,stdvm);

	fprintf(fp_out,"%s\t",inFileName);
	fprintf(fp_out,"%s\t",comments);
	calculateStats(&avg,&stdvm,0,25,cv_a);
	fprintf(fp_out,"%f\t%f\t",avg,stdvm);
		printf("%2.2E %2.2E ",avg,stdvm);
	calculateStats(&avg,&stdvm,0,25,led_a);
	fprintf(fp_out,"%f\t%f\t",avg,stdvm);
		printf("%2.2E %2.2E ",avg,stdvm);
	calculateStats(&avg,&stdvm,0,25,temp_a);
	fprintf(fp_out,"%f\t%f\t",avg,stdvm);
		printf("%2.2E %2.2E ",avg,stdvm);
	calculateStats(&avg,&stdvm,25,50,MoverI);
	fprintf(fp_out,"%f\t%f\t",avg,stdvm);
		printf("%2.2E %2.2E ",avg,stdvm);
	calculateStats(&avg,&stdvm,25,50,CoverI);
	fprintf(fp_out,"%f\t%f\t",avg,stdvm);
		printf("%2.2E %2.2E ",avg,stdvm);
	calculateStats(&avg,&stdvm,25,50,SoverI);
	fprintf(fp_out,"%f\t%f\n",avg,stdvm);
		printf("%2.2E %2.2E\n",avg,stdvm);
	...
	printf("Useage ./analyzePolVac <input PolVac file> <output file> <0,1>(1:write summary or 0:write individual values)\n");
}
